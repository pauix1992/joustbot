...
def melee(comment):
    ...
    result = melee_recursive(comment,*contestants)
...
def melee_recursive(comment,*arg):
    res=''
    survivors = list(arg)
    if len(survivors) > 2:
        r_min = 200
        ...
        for i in range(0,len(survivors)):
            ...
            print(player[0])
            roll[i] = randint(1,50)+int(player[1])
            clean = int(roll[i])-int(player[1])
            if roll[i] < r_min:
                r_min = roll[i]
            res += player[0] + " rolls a " + str(roll[i])+" (" + str(clean) + " + " + str(player[1])+ ")\n\n"
        ...
        survivors = a
        comment.reply(res);
        return melee_recursive(comment,*survivors);
